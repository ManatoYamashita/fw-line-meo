...
- **短命な Cloud Run Job は確保した資源を必ず閉じる。閉じ忘れは業務ログ上は完全に正常に見える（Issue #151・2026-08-24 是正）**: `ts/packages/db/src/pool.ts` の `createPool()` が本番経路（`cloud-sql-iam`）で Cloud SQL Connector を生成しながら参照を保持せず、`closePool()` が `pool.end()` しか呼んでいなかった。Connector はインスタンス証明書のリフレッシュを ref 付き `setTimeout` で予約する（実装は `@google-cloud/cloud-sql-connector/dist/mjs/cloud-sql-instance.js` の `scheduledRefreshID` / `cancelRefresh`）ため、閉じない限りイベントループが空にならない。`summary-delivery` は業務処理を **2 秒で完走してサマリー行まで出した後**、プロセスが終了できず 600 秒でキルされ、リトライしてまた同じことを繰り返していた（所要 21 分・直近 60 execution の成功数はゼロ）。**長命な Cloud Run サービス（`survey-web` / `line-webhook` / `dashboard-api` / `store-detail`）は同じコードでも顕在化しない** — プロセスが終わらないことが正常だからである。露出するのは Job だけ。したがって是正は「Pool を返す」ではなく **「Pool と閉じ方の対（`PoolHandle`）を返す」** 形にして、解放を呼び出し側の記憶に委ねない。構築途中で失敗した経路（`getOptions()` の reject）でも閉じる。
  - **回帰ガードは「メソッドを呼んだか」ではなく「プロセスが終われるか」を固定する。** `ts/packages/db/test/pool-connector.test.ts` は `process.getActiveResourcesInfo()` の `'Timeout'` 件数が基準値へ戻ることを見る。`getPool()` 直後に基準値 +1 であることも対にして確認する（掴んでいない状態で「解放できた」と言うのは空振りである）。`cloud-sql-iam` 経路は CI・ローカルの全テストで一度も実行されない（すべて `DATABASE_URL` 経由）ため、フェイク Connector を `vi.mock` で差し込む以外に検証手段が無い。
  - **アラートポリシーを `job_name` で絞らない。** 撤去前のポリシーは `resource.labels.job_name = "daily-batch"` に固定されており、後から追加された `summary-delivery` を見る監視が 1 つも無かった。Monitoring の時系列は `result="failed"` を日次 22〜24 件すでに計上していたのに鳴らなかった、つまり**無音の原因は述語ひとつ**である。ジョブを足すたびに配線を思い出す設計は、思い出さなかったときに無音になる。述語を持たなければその忘れ方は起き得ない（ガードで守るのではなく、守る対象を消す）。リネームは `moved` で対応付けて destroy/create を避ける — 監視を直す変更が、その隙間に発生した失敗を落としては本末転倒である。
  - **これは #125 の「execution 成否を証拠に採ってはならない」の逆側の失敗である（下記の同項を参照）。** サマリーログ行だけを見ていれば正常に見え、execution 状態を見て初めて分かる。#125 は execution 成否だけを見ると騙されると言い、本件は業務ログだけを見ると騙されると言う。**片方だけでは足りない。** 完了の証拠には両方を並べる。
...
### UI の a11y 検証規律（Issue #53）
- **自動 a11y 監査（axe / Lighthouse）と実測の assert は役割が違う。前者を入れても後者を削らない。**
  axe は「誰も書かなかった項目」を汎用規則で拾う装置で、aria・role・ラベル・コントラストを見る。
  一方 **フォーカス指標の視認性には axe の自動規則が無い**（axe が「手動確認」に分類している領域で、
  axe-core を内包する Lighthouse も同じ限界を持つ）。実際、フォーカス可視性を失わせた欠陥へ
  両者を当てても緑のままで、`getComputedStyle` で outline を実測する E2E だけが赤くなった。
  自動監査を「a11y は機械が見ている」と読み替えた瞬間に、この穴が無防備になる。
- **a11y 監査は「面が本体を描けている」ことを先に固定してから当てる。** 空の画面・エラー画面には
  違反が出ようがないため、前提を置かない監査は面が壊れたときに最も静かに緑を返す。実測でも、
  エラー分岐の 1 段落だけの DOM が違反 0 件・規則 5 件で通り、「規則 0 件」検出まで満たした。
  開く手順と前提 assert は面ごとに `e2e/fixtures/` の 1 箇所へ置き、監査する spec は必ずそれを経由する
  （`page.goto` を監査 spec へ直書きしない）。同じ面を測る他の spec があるなら、その定義を共有する。
  **この規律は `scripts/check-a11y-audit-preconditions.sh` が機械強制する。** 文章だけでは守れない
  ことが実証済みで、規律を書いた当の PR が 9 経路のうち 2 経路で破り、CI は全緑を返していた。
- **違反 0 件と規則 0 件を区別する。** 対象セレクタが外れる・注入が失敗するといった経路では、
  axe は例外ではなく空の結果を返す。「何件の規則が実際に評価されたか」を併せて要求する。
  **この半分も `scripts/check-a11y-audit-preconditions.sh` が機械強制する。** 監査ヘルパが 1 件だけ
...
ソースや CSS から検証対象を拾う **抽出器** を書いたら、fixture で入出力を固定するテストを必ず添える。

- **「空振り防止」と「自己検証」は別物である。** 非空 assert（`extracted.size > 0` / `検証表が空でない`）が
  保証するのは **対象ゼロで緑にならないこと** だけで、**拾ったものが正しいか**は一切見ない。
  位置依存の取りこぼしと、宣言でないものの誤検出はそのまま素通りする。PR #59 では、抑制ブロックの
  **先頭**に置いた宣言だけが判定から漏れていた（非空 assert は 3 件返るので緑のままだった）。
- **固定する性質は 2 方向ある。** 「拾ってはならないものを拾わない」（誤検出）と
  「拾うべきものを漏らさない」（取りこぼし）。片方だけでは #48 で実際に空振りした。
  取りこぼし側は **先頭・末尾・入れ子** の位置を必ず含めること。実測（#60）: `standaloneHeadingRules` は
  「Preflight のセレクタリストを拾わない」と実装コメントで主張していたが、**末尾レベル（h6）で破れていた**。
  先頭レベル（h1）は直後が `,` なので偶然通り、利用側も末尾要素を採っていたため順序の偶然に守られていた。
- **CSS を機械検証するときは postcss の構文木を使う。** `プロパティ: 値;` の正規表現による宣言抽出を
  新規に書かない。CSS には「宣言と同じ形をしていて宣言でないもの」が最低 2 種類ある
  （`@media (cond: value)` の条件・`sel:pseudo` の選択子）。セレクタの同一性やレイヤ所属も
  文字列の見た目では判定できない。
- **名前で範囲を切り出す抽出器は前方境界を要求する。** `indexOf(`${group}: {`)` や
  `new RegExp(name + ':')` は接頭辞・接尾辞が一致する別の名前を掴む
  （`--color-text` ↔ `--color-text-muted`、`variant` ↔ `subvariant`）。
  `(?:^|[;{\s,])` のような境界を前に置き、**両方向の衝突を fixture で固定する**。
- **ガードの赤化実証では注入位置・順序を最低 2 通り試す**（先頭と末尾、最初と最後の要素）。
  1 箇所での赤化は位置依存の穴を隠す。
- **同じ教訓の実装を 2 箇所に分散させない。** PR #59 の根本原因は、「文字列の見た目で判定しない」という
  同一の教訓が `app-integration.test.ts`（AST）と `components.test.tsx`（正規表現）で別実装に
  なっていたことだった。
- **自己検証そのものが的を外していないかを変異で確かめる。** #60 の作業中、`--brand-color-a` を使った
  境界の fixture がハイフン 1 本のため対象の部分文字列を含まず、境界を外す変異で赤にならなかった
  （`--brand--color-a` が正しい）。**fixture を書いたら、それが守るはずの性質を壊す変異を 1 つ当てる。**
...
- **`set -euo pipefail` の下で、入力を読み切らない consumer をパイプ終端へ置かない。**
  ...
  **入力が単一行なら発火しない。** grep は行単位で判定するため、1 行しか来ない入力では行末まで
  ...
  - 機械強制は `scripts/check-shell-pipe-consumers.sh`（Issue #117）。追跡下の `scripts/**/*.sh`
    ...
    `sed` がパイプの下流に現れる行を弾く。除外は 3 つある。行頭 `#` のコメント行は**全ファイル**、`WHITELIST=(` の
  ...
  - 後半（後置 `true` の禁止）の機械強制は `scripts/check-grep-exit-codes.sh`（Issue #120）。
    同じく `scripts/**/*.sh` と `db/test/**/*.sh` を走査し、`grep` を含む行が後置 `true` で
    失敗を潰していないかを見る。導入時に 43 箇所を変換済み。**走査面を広げるときは、新しい面へ
    違反を置く赤ケースだけでは足りない。** 面を pathspec から外す変異を当てると違反ごと見えなく
    なって緑を返すため、「走査していない」と「違反が無い」を区別できない。**母数の照合**
    （`expect_output_matches '2 ファイル / 2 件の … を検証'`）を別ケースとして置くこと（#162）。**`grep` と後置 `true` が別の行に分かれた複数行パイプラインは
    検出できない**ため、そこは人手の規律のまま残る。
- **dry-run の注入変数は `${VAR+x}` で判定する。`${VAR:-}` は空文字を「未設定」と同義にする。**
  分岐を実証するための注入は、**空文字そのものが意味を持つ**（「該当なし」「前回署名なし」）。
  `[ -n "${VAR:-}" ]` で見ると空文字が偽になり、注入分岐へ入らず**実 API を叩く本来の経路へ落ちる**。
  実測（Issue #108・`scripts/report-ci-issue.sh`）: `REPORT_CI_ISSUE_DRY_RUN=1` かつ
  `REPORT_CI_ISSUE_FAKE_TRACKER=''` で `gh issue list` が実際に 1 回呼ばれていた。
  **症状は出力に出ない。** 探索は `tracker_list="$(gh issue list … 2>&1)"` とコマンド置換で
  呼ばれるため、失敗の診断まで変数へ吸われ、数字行が 0 件になって「追跡 Issue はありません」という
  **意図どおりの表示**になる。旧版と是正版の出力は 1 文字も違わず、違いは `gh` 呼び出し 1 件と 0 件だけだった。
  実害は 2 つ。(1) 「追跡 Issue 有り」を意図したテストが `issue create` 経路を通って**偽 PASS** する。
  (2) dry-run が実 API へ read を飛ばし、ネットワークやトークンの状態でテストが落ちうる。
  - **自己テストのスタブが不具合を迂回しうる。** 当時のスタブは `issue list` だけ素通しさせて
    「該当なし」を作っていたため、注入が効いていなくても意図どおりに見えた。**起動の有無そのもの**を
    見るには、探索も含めて全ての起動を赤にするスタブが要る。
  - 同一ファイル内に注入点が複数あるなら**記法を揃えたまま保つ**。`report-ci-issue.sh` は
    FAKE_PREV_SIGNATURE 側だけ `+x` で正しく、記法の食い違いがそのまま挙動の食い違いだった。
    揃っていることは `scripts/test/cases/70-report-ci-issue.sh` が件数で機械照合する。
...
### spec の実施記録の規律（`.kiro/specs/**/tasks.md`）

実施記録は「後から読む人が、実施していないことを実施したと誤読しない」ために書く。
以下はいずれも PR #150 のレビューで実際に踏んだ形（残余の追跡は Issue #152）。

- **本番の識別子を実値のまま書かない。先頭 8 文字までに切り、切った理由を同じ段落へ残す。**
  このリポジトリは公開されており、記録は commit として永続する。店舗 ID は客向けアンケート経路
  （`/s/<storeId>` の SSR から `pageToken` を取り `POST /api/responses`）へ到達する唯一の関門で、
  `place_status = 'confirmed'` の店舗 ID を知れば誰でも匿名集計へ加算できる。照合用途には先頭
  8 文字で足りる（保存ファイル名・`aria-controls`・パネルの `id` はいずれもこの断片を使う）。
  理由を書き添えないと、後任が「短くする意味は無い」と判断して実値へ戻す。
  push 済みなら `git commit --amend` ＋ `--force-with-lease` で実値を `main` へ入れない。ただし
  **force-push 後も GitHub は旧 commit を SHA 到達可能なまま保持する**。実値を一度 push した時点で
  漏洩は成立していると扱い、対象そのものの入れ替えまで検討すること。
  **識別子は文字列とは限らない。** 実物の QR 画像はその URL を符号化しており、画面の写真にも
  写り込む。記録へ添付する成果物は、値が復号・判読できないかまで見てから置くこと。
- **Observable を部分的にしか満たしていないのにチェックを閉じるなら、追跡 Issue をタスク本体から指す。**
  チェックボックスが閉じた瞬間に spec は完了と読め、残余を追える面がリポジトリから消える。参照は
  **チェック行の直下**（`- [x]` を読んだ人が必ず通る位置）と末尾の実施記録の両方へ置く。記録側にだけ
  書いても、進捗を見る人はそこまで読まない。残余は Issue 側の完了条件へも写し、片方だけが正典に
  なる状態を作らない。記録本文では「確認した」と「満たした」を混ぜず、未実施は未実施と書く。
  検証を実物ではなく代理（構造・静的解析・模擬）で行った場合は、**何を代理したかと、代理では
  埋まらない差**を必ず併記する。
- **起動手順が要求する env は「env 宣言表」で書く。散文へ埋めない。**
  書式は `| env | 出典 | ローカルでの値 |` のヘッダを持つ表で、第 1 列はバッククォートで囲んだ
  env 名、第 2 列はその env を読む実装ファイルの**リポジトリ相対パス**（直前行と同じなら `同上`）。
  `scripts/check-spec-env-names.sh` がこの表だけを抽出源として両方向で照合する — 表の env 名が
  出典に実在すること（文書 → 実装）と、出典の `throw new Error('<NAME> is required')` がすべて
  表に載っていること（実装 → 文書）。Issue #148 は、4.2 の手順が実在しない `CORS_ORIGIN` を
  指示し、必須の `SURVEY_BASE_URL` と `PLACES_API_KEY` を落としていた形である。
  抽出源を表へ限るのは、素の大文字トークンが実測で 93% は env ではないためであり、
  **散文中の env 名は意図的に見ない**。是正済みの文書は「その名前は存在しない」と教えるために
  誤った名前を本文へ残しており、そこを叩くガードは最も正しい記述を罰する。
  実装側は必須 env を `throw new Error('<NAME> is required')` の形で自己申告すること。
  ヘルパ経由（`` Error(`${key} is required`) ``）にすると、その出典は照合の対象から静かに外れる。
