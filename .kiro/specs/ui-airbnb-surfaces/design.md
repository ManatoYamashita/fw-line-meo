...
- 共通部品そのものの追加・変更。不足が判明したら部品側の課題として差し戻す。ただし、レビュー後に
  利用者が本 PR 内での解消を指示した Issue #204 の既存 `Button` `lg` 契約訂正は例外とする
...
- PR #200 のレビューで確定した Issue #204 に限り、既存 `Button` の `lg` 区分を正典 §7.10 の
  既存拡大区分契約へ訂正し、部品単体と本番面の実描画で固定すること
...
- `ts/packages/ui/**` と `ts/packages/design-tokens/**`。**部品もトークンも書き換えない**。
  ただし Issue #204 の `Button` `lg` 区分と、その回帰検証だけはレビュー後の境界例外とする
...
- 管理ダッシュボードと店舗詳細への Playwright 設定・E2E ディレクトリ・CI ジョブの**新設**
  （**新設はしない。ただし PR #190 が新設した後の追随は範囲に含む**。下記「実装中に動いた境界」）
- LINE 面（`ts/apps/line-webhook/**`・`ts/apps/delivery-job/**`）
- API・データモデル・認証・インフラ。1 行も触れない

### 実装中に動いた境界（2026-09-06 記録）

境界は起草時の想定どおりには保たれなかった。**動いたのは 4 点である。先の 3 点は上流の着地が
原因であり、本 spec の都合で広げたものではない。4 点目は PR レビュー後の利用者指示である。**
Revalidation Triggers のうち 3 つが実際に発火している。

| 動いた点 | 何が起きたか | 扱い |
|---|---|---|
| 管理ダッシュボードと店舗詳細の `e2e/` | PR #190 が足場を新設した（トリガー「2 面に実描画検証が導入されたとき」の発火） | **追随を範囲に含めた。** 捲れる領域の宣言（帯 / 推移表）を実測へ合わせる変更のみ。新しい面も新しいジョブも作っていない |
| `ts/apps/dashboard-web/**` の実装 | Issue #45 が PR #196 で先にクローズされた | **本 spec の段階 1・4 の実装は破棄し、`origin/main` を正とした**（下記） |
| `docs/design/design-language.md` §7 の節番号 | PR #194 が LINE 面の判断を §7.8〜7.12 として先に入れた | 合流時に **LINE 側を §7.12〜7.16 へ繰り下げ**、Web 判断（7.1〜7.11）と LINE 判断（7.12〜7.16）が連続するようにした |
| `ts/packages/ui` の `Button` `lg` 区分 | PR #200 の実描画レビューで、主操作が副次入力より小さいことが判明し、Issue #204 へ差し戻した。その後、利用者が F2 として本 PR 内での解消を指示した（トリガー「共通部品の既定 className の変更」の発火） | **既存区分の契約訂正だけを範囲に含めた。** 新しい区分・公開 API・依存は追加せず、正典 §7.10 の既存拡大区分契約を部品側で持たせる |

**dashboard-web の実装が #196 へ収斂したことの意味。** 本 spec の tasks 2.1〜2.5・5.1・5.2 の
実施記録は残してあるが、**そこに書かれた実装そのものは出荷されていない**。出荷されたのは #196 の版で、
ソースは 300 行以上異なる（jsdom テストの側はほぼ同一だった）。記録を残すのは、赤化の実証と
判断の経緯が後から参照される資産だからであり、**実装の出典として読んではならない**。

唯一 #196 に無く本 spec から運んだものは、段階 6 の CI（run 34003887862）が実測であぶり出した
競合の是正 3 箇所である（`await screen.findByRole('main')` を取得の完了と読み違えていた形）。
...
- 店舗詳細に 0 件の状態を解消する操作が導入されたとき（Issue #152 の口コミ QR 発行 UI または第 2 フェーズの
  競合範囲設定）。空状態の文言と導線の判断を見直し、リンク個数を固定する検査も更新する
...
**この集合が閉じているのは「役割の既定から外れる色を面の側で選び直す箇所」についてである。**
補足の文へ補足色を当てるような、役割どおりの適用は含まない（そこは色を選び直しておらず、
部品の内側でも同じ役割が同じ色で使われる）。限定を書かずに読むと、面の側の色ユーティリティが
1 つでも増えた時点で違反に見えるが、実際に禁じたいのは**面ごとに色の役割を作り直すこと**である。
段階 3 の完了時点で、この規律に照らした 3 面の実態は次のとおりで、いずれも違反していない。

| 面の側の色ユーティリティ | 分類 |
|---|---|
| 帯とログインのワードマーク（装飾専用色） | 下の表の 1・2 行目 |
| 客向けアンケートの星 2 色 | 下の表の 4 行目 |
| 下書きの処理中の文言・回答不可の案内・店舗一覧の補足文（いずれも補足色） | 役割どおりの適用（対象外） |
| 各面の外枠が持つページの既定（背景色と本文色） | 役割どおりの適用（対象外） |

**下の表の 3 行目（順位の巨大表示）は、実装では色ユーティリティを持たない。** 外枠が与える
本文色をそのまま継承しており、面の側は寸法と太さしか指定していない。行を残してあるのは、
`docs/design/design-language.md` の 2.2 節が本文色とページ背景の組の出典としてこの表示を挙げており、
実効コントラストの記録がそちらに要るためである。**列挙されている ≠ 面の側が色を書いている**。
...
**以下の dashboard-web の計画は、実装としては出荷されていない**（上記「実装中に動いた境界」）。
設計としてどう当てるつもりだったかの記録として残す。判断そのもの（版面の置換・主要領域を
入れ子にしない・選択を包む段落を容器へ置き換える）は #196 の版にも通っている。
...
├── fixtures/surfaces.ts            # 面を開く手順の所有者（PR #191 が新設。本 spec は消費側）
...
- `ts/apps/store-detail/app/store/page.tsx` — 版面・順位の巨大表示・カード化・推移表の部品化。
  書込操作の要素を 1 つも足さない
  - 2026-09-13 訂正（Issue #265）: 店舗詳細は、書込の手段となる要素を描かず、入力を許可リスト（期間・指標の選択肢と競合の検索欄）に限る形へ改定した。詳細は requirements.md の要件 3.1・3.3 の訂正にある
...
**無し（訂正・2026-09-06）。** 起草時は `ts/apps/survey-web/e2e/support/viewport.ts` を新設する
予定だったが、同じ切り出しが PR #187 で `ts/packages/e2e-support`（`@fwlm/e2e-support/viewport`）
として先に着地した。**上流の判断なので本 spec の成果物は破棄し、消費側へ回った**（task 1.4 の
実施記録に経緯がある）。ロジックは同一で、消費者は survey-web / dashboard-web / store-detail の
3 面になっている。

面を開く手順についても同様に、PR #191 が `e2e/fixtures/` を単独の所有者と定め、
`scripts/check-e2e-goto-ownership.sh` が spec 側の `page.goto` を機械的に禁じた。
本 spec が段階 5・6 で足した検証も、この所有権に従って fixtures 経由で面を開く。
...
| 2.3 | `EmptyState` に導線を children で渡す。`role` は既定で持たない。**店舗詳細には提示すべき操作が存在しないため導線を持たない**（競合設定は第 2 フェーズ・推移は翌朝のバッチ・口コミ QR に発行 UI が無い）。個数を固定する 3.1 / 3.3 はその不在を構造で保証しているだけで、緩めても導線は現れない。同面では「推移データはまだありません（毎朝の集計後に表示されます）」「新着なし（前回の集計以降、新しいクチコミはありません）」のように、次回更新を待つ状態であることまで空状態で伝える |
| 2.4 | `Table` が縞も重畳塗りも持たない。実描画で面塗りの不在を要求する既存 E2E がある |
| 2.5 | `TableContainer` の横方向の捲り。表の外側に置く |
| 3.1 | 店舗詳細画面に部品を選ぶ際、書込操作の要素を持つ部品を使わない。2026-09-13 訂正（Issue #265）: 店舗詳細は、書込の手段となる要素を描かず、入力を許可リスト（期間・指標の選択肢と競合の検索欄）に限る形へ改定した。詳細は requirements.md の要件 3.1・3.3 の訂正にある |
...
**~~要件 4.6 は 2 面で機械検証できない。~~ 解消済み（2026-09-06）。** 起草時は管理ダッシュボードと
店舗詳細に実ブラウザで幅を測る足場が無く、目視確認の記録で補う想定だった。**実装中に前提が変わった。**
PR #190（Issue #53）が両面へ Playwright の足場を入れ、`@fwlm/e2e-support/viewport` を 3 面で共有する
形になったため、要件 4.6 は 3 面すべてで機械検証される（管理 6 面 + 店舗詳細 1 面 + 客向け 2 件）。
目視確認へ退避する必要はなくなった。task 7.2 がこの再評価を行い、記録を残している。
...
**要件 1.2 は 3 面すべてでは実証されないまま残る（縮小・2026-09-06 再評価）。** 起草時の理由
「2 面に実描画検証が入るまで手段が存在しない」は PR #190 で解消したが、**入った足場が測るのは
横スクロールと axe の汎用規則であって、面をまたいだ同一描画ではない。** 実際に固定できたのは
客向けアンケートの面内の相等（回答済み画面 ↔ 下書きパネルの投稿導線が同一である旨を
`survey-shell.test.tsx` が assert する）までである。3 面をまたいだ同一描画を実測するには、
同じ役割の要素を面をまたいで指す共有の識別子が要る。**それは検証基盤ではなく部品側の課題**であり、
本設計の境界の外にある。
