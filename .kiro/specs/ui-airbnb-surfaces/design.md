...
- PR #200 のレビューで確定した Issue #204 に限り、既存 `Button` の `lg` 区分を正典 §7.10 の
  48px / 16px へ訂正し、部品単体と本番面の実描画で固定すること
...
| `ts/packages/ui` の `Button` `lg` 区分 | PR #200 の実描画レビューで、主操作が 36px / 14px のまま副次入力より小さいことが判明し、Issue #204 へ差し戻した。その後、利用者が F2 として本 PR 内での解消を指示した（トリガー「共通部品の既定 className の変更」の発火） | **既存区分の契約訂正だけを範囲に含めた。** 新しい区分・公開 API・依存は追加せず、正典 §7.10 の 48px / 16px を部品側で持たせる |
...
| 2.3 | `EmptyState` に導線を children で渡す。`role` は既定で持たない。**店舗詳細には提示すべき操作が存在しないため導線を持たない**（競合設定は第 2 フェーズ・推移は翌朝のバッチ・口コミ QR に発行 UI が無い）。個数を固定する 3.1 / 3.3 はその不在を構造で保証しているだけで、緩めても導線は現れない。同面では空である旨の提示までとする |
