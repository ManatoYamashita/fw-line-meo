...
- **In scope**: 店舗一覧の各行からの QR 発行操作への到達／発行した QR の画面上での確認／画像ファイルとしての保存と、店舗を判別できるファイル名／店舗の場所が未確定である場合の理由提示／発行失敗時の提示と再試行／発行導線のキーボード操作・支援技術対応・視認性／**単一店舗ぶんの店頭掲示用の面と、そこに載せる規約準拠の文言（Requirement 7・Issue #179 で追加）**
...
> Requirement 7 は上の Out of scope に触れない。除外されているのは **複数店舗の一括発行と
> 印刷面付け（PDF 等）** であり、Requirement 7 が扱うのは単一店舗ぶんの 1 枚である。PDF を
> 自前で組み立てることもしない（ブラウザの印刷機能へ渡すだけで、紙にするか PDF にするかは
> 利用者が選ぶ）。掲示物そのものは元から想定用途であり、`research.md` は「受け入れ基準は
> 卓上 POP と店頭掲示の双方を含む」と記して QR の一辺を 1024 px に固定する根拠にしている。
...
### Requirement 7: 店頭掲示の文言提供

**Objective:** As a 代理店・運営, I want QR と一緒に規約準拠の掲示文言が届くこと, so that 店頭に貼る紙の文言を現場の判断に委ねずに済む

#### Acceptance Criteria

1. When 店舗の QR 発行が成功したとき, the 代理店ダッシュボード shall その QR と同一の画像を用いた店頭掲示用の面を提示し、規約に準拠した依頼文と読み取りの案内を含める
2. The 代理店ダッシュボード shall 掲示用の面に表示する依頼文を、投稿の見返りとなる特典に言及せず、評価の高低にも書く内容にも触れない形で提供する
3. When 利用者が掲示用の面の印刷を要求したとき, the 代理店ダッシュボード shall 掲示用の面のみを印刷対象とし、操作要素と後述の不可の例を印刷対象に含めない
4. The 代理店ダッシュボード shall 掲示してはならない文言の例を、それぞれが規約上どの禁止に当たるかとともに画面上で示す
5. The 代理店ダッシュボード shall 掲示用の面に来店客に関する情報および評価の集計結果を表示しない
6. If QR の発行が完了していないとき, then the 代理店ダッシュボード shall 掲示用の面と印刷の操作のいずれも提示しない

> 本要件は Issue #179 で追加した。2026-04-17 の Google のポリシー改定で「評価の操作」が新設され、
> 「その場（店内）での評価の要求・強要」と「特定のコンテンツを含めるよう依頼すること」が
> 販売者の禁止事項として明記された。**QR の飛び先は店舗ごとに一意で、そこから投稿される先は
> その店の Place に固定されている。** したがって掲示文言が「星5でお願いします」であった場合、
> システム側の下書き生成がどれだけ厳格でも掲示物 1 枚で違反が成立し、制裁は当該ビジネス
> プロフィールに着弾する。IT に不慣れなオーナーが対象顧客である以上、文言の判断を現場に
> 委ねるのは設計上の欠陥である。
>
> 受入基準 3 が「不可の例を印刷対象に含めない」を明示するのは、紙に不可の例が刷られると
> **この要件が防ごうとした違反そのものを製品が配ることになる**ためである。
>
> 判断の根拠は原文であり二次情報ではない（`CLAUDE.md` の制約）。原文は
> `https://support.google.com/contributionpolicy/answer/7400114`。**募ること自体は明示的に
> 許可されている**（`Solicit or encourage the posting of content that does represent a genuine
> experience, without offering incentives`）ので、依頼文は誘い掛けに留める。
