...
- **既知の限界**: 不可視面は操作可能部品どうしの関係だけを検証している（要件 4.5 の文言に忠実）。押しボタンに近接して置かれた**操作可能でない要素**（本文テキスト等）の上に拡張面がかぶさると、その範囲では文字選択などが妨げられうる。`Checkbox` / `RadioGroupItem` が以前から抱えていた性質と同じであり、本 spec で新たに生じた問題ではない。`/ui-check` の間隔（16 ピクセル）に対し拡張は片側 7 ピクセルなので現状は接触しない。各面のレイアウトで部品を極端に詰めた場合は #43〜#45 側で確認すること。
