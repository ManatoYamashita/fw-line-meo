...
db/migrations/0013_gbp_post_review_reply.sql  # gbp_locations・gbp_sessions 新設（COMMENT 込み）
```
...
### Physical Data Model（migration `0013_gbp_post_review_reply.sql`）
...
1. **DB**: `0013_gbp_post_review_reply.sql` 適用（`make db-migrate` / `db-test` / `db-verify-docs` を通す。ERD・write-boundary 同時更新）
