...
- **Web**（`ts/apps/dashboard-web`）
  ...
  - `e2e/fixtures/api.ts` — `openUserEditPanel` と `DASHBOARD_SURFACES` への登録。「7 面」を 8 面へ直す。
  - `e2e/dashboard-surfaces.spec.ts` — 編集パネルの状態の横スクロール実測を 1 本足す。
...
- **変更の算出**（3.1, 3.4）: 開いた時点の `user` と比べる。
  ...
  - displayName: trim して空なら null にした値が、開いた時点と違う場合に送る。
