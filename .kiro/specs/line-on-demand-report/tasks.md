...
- [x] 7.5 実機で確かめる
  ...
  - **部分達成で閉じた。** 新着口コミの抜粋の個別描画（アバター・投稿者リンク・本文切り詰め）の実機確認は、本番に条件を満たす店がほぼ無いため残した。残りは #292 が追う（実施記録の 7.5 を参照）
...
- （4.3）要件 3.5（停止中の店舗を除く）は、店舗の利用停止（#252）が未導入なので今は実装できない。要件自体が「先に導入されていれば」という条件つきで、述語の置き場所を `targets.ts` のコメントで名指ししてある（2026-09-23 追記: `store-suspension` の 3.2・3.4 で `targets.ts` と `report-reads.ts` に停止の述語を足し、要件 3.5 は満たされた）
...
### 7.5 実機で確かめる（2026-09-19・**部分達成**）

既存の検証用テナントで行った（本番の店舗とオーナーは消せないので、新しくは作っていない・#252）。確定店舗を 2 つ持つオーナー（`4c89b0e6`）が実在したので、複数店舗の選択まで確かめられた。本番の稼働イメージは `eff6838`。

**確かめたこと**

| 項目 | 結果 |
|---|---|
| 完了後メニューの 5 区画 | 上段 3（新着口コミをみる・競合店との比較をみる・直近の推移を見る）／下段 2（詳細を見る・ステータス確認）。チャットバーは「メニュー」 |
| 複数店舗の選択 | レポートの要求に店舗名の選択肢が返り、選んだ店舗のレポートが届く。タップした文言がオーナーの発言として表示される |
| 新着口コミ | 「新着口コミはありません」。前日の集計があるので「判定できません」ではない（4.5 の出し分けが効いている） |
| 競合店との比較 | 6 店中 3 位・★4.1（クチコミ 328 件）・競合 5 店と星差。DB の実値と一致 |
| 直近の推移 | 7 暦日の表と要約 1 行、詳細画面への導線 |
| ステータス案内 | 3 行。「毎朝」「毎日」の約束が無い（3.8 の文言） |
| LIFF の店舗のヒント | **効いている**。複数店舗のオーナーでも選択画面に落ちず、指定した店舗が出た（6.6） |
| 帰属表示 | レポートの各バブルの末尾に「データ提供: Google Maps」 |
| 評価のない競合 | ★0 にならず末尾に置かれ、詳細画面に「評価のない店は順位に含めていません」が出る（#255 の是正が実機で効いている） |

**応答時間**（本番のリクエストログの実測・`httpRequest.latency`。体感ではなくサーバー側の値）

| 回 | 応答 |
|---|---|
| 初回（コールドスタート） | **3.229 秒** |
| 2 回目以降（6 回） | 0.077〜0.235 秒 |

判断基準の 5 秒以内なので、**line-webhook の最小インスタンスは引き上げない**（9.6 の判断）。

**見つけた不具合 2 件**

- **推移レポートの列見出しが「クチコ…」と切り捨てられていた**（本 spec の欠陥）。値のセルは `wrap` を持つのに、見出しのセルだけが持っていなかった。列が何の数字か読めなくなる。PR #293 で是正した（見出しにも `wrap` を足し、変異で赤を実測した）
- **店舗詳細（LIFF）の推移表で、クチコミ数が「227」と表示され、実値 2274 と読めない**。横捲りの右端が隠れ、切れた数字がそれ自体で意味の通る別の数に見える。本 spec の範囲外（#265 の成果物）なので #286 へ記録した

**確かめられなかった 2 点（データ待ち）→ #292 が追う**

- **新着口コミの抜粋**（投稿者の画像・Google Maps 導線・本文の切り詰め）: 実施時点で 3 店舗すべて・直近 9 日分すべてで `new_review_count = 0` だった。抜粋そのものが出ないので、8.6 の「画像が描けなければ外す」判断ができない
- **変化があった日の通知の到達**: 変化が無く、当日分（2026-09-19）は旧コードの朝の配信で `delivered` として記録済みだった（`UNIQUE (store_id, summary_date)` により同じ日には再判定されない）

タスクは**部分達成**である。残余は #292 が追い、Go が帰属 3 項目を書いた行（2026-09-20 以降のバッチ）で確かめる。

> **【訂正 2026-09-20・Issue #303】上の「データ待ち」は前提が誤っていた。** 抜粋は待っても来ない。
> Places API (New) の `Place.reviews` は関連度順に最大 5 件しか返さず、新着順へ並べ替える手段が無い
> （原文 "sorted by relevance. A maximum of 5 reviews can be returned."）。本番の 3 店舗へ実際に叩くと、
> クチコミ 2291 件の店で返る 5 件のうち最も新しいものが約 3 か月前だった。日次バッチは「前日より後」を
> 抜粋に採るので、クチコミ数の多い店では**当たる余地が構造的に無い**。実測でも全 67 行・全期間で
> `new_reviews` は空配列（同じ期間の新着の総和は 22）。
>
> したがって 8.6（画像が描けなければ外す）の実機確認は、この経路では成立しない。クチコミ総数が
