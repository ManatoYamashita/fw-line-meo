...
- 既存の LIFF（2026-09-20 に是正済み・#287／PR #301）: 口コミを Google Maps への導線なしで表示し、帰属表示の色も本文色（`#222222`）を継承してポリシーの許す 3 値の外にあった。大きさ（14px）だけは 12〜16sp の範囲に収まっていた。是正は表示層だけで閉じている（帰属 3 項目は本 spec の Go 側の変更で既に書かれ、`daily_summaries.new_reviews` の jsonb ごと画面まで届いていた）。内容を出してよいかの判定は `@fwlm/db/daily-summary` の `isDisplayableNewReview` に寄せ、本 spec の新着口コミのレポートと LIFF が同じ述語を読む
