...
- VariationSeed: 文体・書き出し・切り口の候補リストからサーバーがランダム選択（3.3 の実務担保。research.md 参照）
  - 書き出し・切り口の各候補は、成立に要る素材（なし／特定の観点／観点が 1 つ以上／観点が無い素材だけ／一言）を宣言する。`pickVariation(material)` は素材に対して成り立つ候補だけから選び、どの次元にも素材に依存しない候補を 1 つ以上置く（Issue #254）。候補の指示が素材に無い情報を求めると、その情報が創作されるため（3.2 の違反）。絞る条件は素材の有無だけで、中身は見ない
  - 選ばなかった観点を code で持たない旧 sessionToken では、観点に依存する候補を選ばない（素材に無い情報を求める側へ倒さない）
  - 文体は素材に依存しないので、多様性は文体で確保する（薄い素材では書き出し・切り口の候補が 1〜2 つに減るため）。文体の候補は文の形だけを変えるものに限り、事実を求める指示・感情の強さを変える指示（3.5 に反して不満を和らげる）・字数を縮める指示を入れない
  - systemInstruction は、星の評価を数値で書かないこと、素材で「なし」の項目を「無かった」と断定しないことを求める（どの候補のもとでも、星の読み上げによる定型文と、客が言っていない否定の創作を防ぐ）
