...
- [x] 4.5 (P) 下書きパネル UI（葉コンポーネント）を実装する
  ...
  - 下書きの近傍に由来（客自身の回答をもとに作成したこと）と推敲の促しを、読み上げ領域とは別の静的な段落として表示し、入力欄から `aria-describedby` で指す（Issue #179 で追加。文言に「AI」の語は出さない）
  - Observable: コピー→writereview URL 遷移、低評価でも同一導線、生成失敗時に再試行 UI と投稿導線が残ることがテストで緑
  - _Requirements: 3.6, 3.7, 3.9, 4.1, 4.2, 4.4, 4.6, 4.7_
...
- [x] 6.3 E2E フロー（Playwright・Gemini モック）を実装する
  ...
  - 由来と推敲の促しが実ブラウザで可視であること、および下書き画面が axe の WCAG A/AA 監査を通ることを含める（Issue #179 で追加。それ以前は下書きフェーズが a11y 監査の対象外だった）
  - Observable: Issue #3 完了条件の一連フローが E2E で緑、star による導線分岐が存在しないことを確認
  - _Requirements: 2.9, 3.9, 4.1, 4.2, 4.3, 4.4, 4.6, 4.7_
  - _Depends: 4.4, 4.5_
...
- **[Task 3.1 実装知見] Next.js 16 monorepo**: Turbopack がワークスペースルートを誤推論するため next.config.ts に `turbopack.root` と `outputFileTracingRoot` を `import.meta.dirname` 由来（cwd 非依存）の ts/ で明示する。survey-web の tsconfig は Next 標準（moduleResolution=bundler・jsx は next build が react-jsx に自動設定）で packages/db の NodeNext とは別系統（意図的）。テスト0件の app は `vitest run --passWithNoTests`。docker 不在のため Dockerfile は CI/デプロイ時に検証（起動確認は next start + curl /health で代替）。
- **[Task 6 CI 実走知見] ts-ci.yml の2つの落とし穴（PR #14 で初回 CI 失敗→修正）**: (1) `pnpm/action-setup@v4` は pnpm バージョン源が必要。ルート `package.json` 不在・`ts/package.json` に `packageManager` 未定義だと `No pnpm version is specified.` で全ジョブ即死 → 各 setup ステップに `with: version: 10.33.2` を明示。(2) MSW Gemini モックの `NODE_OPTIONS=--import <mock>` は **job 全体 env に置かない**（checkout 等の全 node アクションを巻き込み `ERR_MODULE_NOT_FOUND`）。モックが必要な実行ステップにのみ、`${{ github.workspace }}` 基準の**絶対パス**で注入する（相対パスは最初の node プロセス=pnpm の cwd がリポジトリルートのため解決不能）。さらに `npx --yes @lhci/cli` は実行時にレジストリ取得するため `--import mock` を被せると npm が起動直後に error 終了する → Lighthouse は `/s/{storeId}` のページ描画のみ計測し SSR は Gemini 非依存（生成は /api/responses のクライアント操作のみ）なのでモック自体が不要。E2E ジョブのみモック注入・Lighthouse は clean で全3ジョブ緑（lint-build-test / e2e / lighthouse=mobile LCP 3秒クリア）。
- **[Issue #62 運用知見] 生成失敗ログの最小診断情報**: `DraftError` は `API_ERROR` に限り終端例外の HTTP `status` を保持し、`/api/responses`・`/api/drafts` は共通ロガーから `errorKind`（必須）と `status`（取得時のみ）を出力する。自由記述・プロンプト・下書き・API キーをログに混入させない。
- **[PR #75 レビュー知見] ログの allowlist は型ではなく sink で担保する**: 当初 `writeStructuredLog` は受け取った fields を `...(fields ?? {})` で spread しており、「ログフィールド型を `errorKind` / `status` の2値に制限した」と記述していた。**これは誤りである。** TypeScript の excess property check は「その場で書かれた object literal」にしか適用されず、変数・関数戻り値・キャスト経由で渡された余剰プロパティは構造的部分型として合法に通る。実測: `{ errorKind, status, comment: '客の自由記述', prompt: '…' }` を `as unknown as SurveyLogFields` で渡すと、型検査は通ったうえで `comment` と `prompt` が 1 行 JSON にそのまま出力された（`test/structured-log.test.ts` で赤化を実証）。是正として sink 側で `errorKind` / `status` だけを明示的に取り出す形へ変更した。あわせて「allowlist にしたことで、`SurveyLogFields` へ項目を足しても取り出しを更新しない限り黙って出力されない」代償を型で塞いでいる（`Exclude<keyof NonNullable<Parameters<SurveyLogger>[2]>, 'errorKind' | 'status'>` が never でなくなると違反鍵名つきでコンパイル失敗。左辺を名前付き型でなく **sink の実引数位置**から導くのは、引数の型を派生型・交差型へ差し替える迂回を塞ぐため）。
- **[PR #75 レビュー知見] `getHttpStatus` の範囲・整数ガードは回帰テストで固定する**: `status` を HTTP の 100–599 かつ整数に限る判定は、外しても既存テスト（400・429・status なし）が全て緑のまま通っていた。ガードが失われると gRPC の status code（例: 8 = RESOURCE_EXHAUSTED）や不正値が HTTP `status` として構造化ログへ載り、「400 ならキー無効」という読み方が成立しなくなる。範囲外（8 / 700）・非整数（429.5）・非数値（`'RESOURCE_EXHAUSTED'` + 数値 `code`）を固定するテストを追加し、範囲ガード除去で 2 件・整数ガードのみ除去で 1 件が赤化することを実測した。
- **[Issue #137 段階3 実装知見] 新テーブル 1 つに機械ガードが 5 箇所ぶら下がる**: `survey_material_tallies`（`0006`）の追加で緑にするために同時に要ったのは (1) `db/write-boundary.md` の表へ 1 行（`check_docs.sh` が「ちょうど 1 行」を要求）、(2) `db/ERD.md` にテーブル名の出現、(3) `infra/sql/grants.sql` の **ヘッダコメントと GRANT 文の両方**、(4) `db/test/assertions/30_compliance.sql` のテーブル allowlist（**外すと `make db-test` が赤**。実測で `FAIL: allowlist 外のテーブル…: survey_material_tallies` を確認）、(5) `db/migrations/` の連番（欠番検査があるため `0006` 以外を選べない）。(1) と (4) はそれぞれ外して赤化することを確認してから入れた。assertions は `70`、smoke は `35` を使った（`60` は未マージの PR #121 が占有しており、先にマージされると衝突するため）。
- **[Issue #137 段階3 実装知見] `aspect_count` に上限 CHECK を書かない**: `survey_aspects` は seed が単一情報源で軸は増えうる（write-boundary.md の共有定数の規律）。`<= 6` を直書きすると軸追加時に INSERT が落ち、しかも呼び手が `tally_failed` で握り潰すため**集計だけが静かに欠ける**。入力の健全性はアプリ側（許可 code のみ受理・重複除去後の配列長を渡す）で担保する。assertions 70e は「負値は拒否し、現行 6 軸を超える 7 は受理する」ことを固定している。
- **[Issue #137 段階3 実装知見] 構造化ログのフィールド追加は 3 点同時**: `SurveyLogFields` の型・sink の明示展開・`EmittedLogField` union。union を忘れると型エラーだが、**sink を忘れると型検査は通り、ログに出ないまま緑になる**。追加時は両方を一度落として挙動を確かめること。
- **[Issue #137 段階3 リリース順序] 本番 migration はデプロイより先**: 本番の migration 適用は手動の out-of-band 作業（`infra/README.md` §3）で、デプロイは main マージ後に自動発火する。厚みの UPSERT は rating/aspect と同一トランザクションなので、**テーブルが無い状態でコードが先に出ると rating/aspect の集計まで巻き添えでロールバックする**（客の体験は Req 5.4 で守られるが、集計が丸ごと静かに欠ける）。`0006` → `grants.sql` の順で適用してからマージする。
- **[Issue #137 段階2 実装知見] 中間層の字数規則は 4 案とも現行を上回らなかった**: 観点ゼロ・一言ありの素材に別の字数規則を与える案（下限なし／100〜200 字／条件つき下限／分類基準の明示）を 160 サンプルずつ実測した結果、**字数を押し上げるのは無条件の下限だけで、それは抽象的な一言に対して創作を呼び戻す**（`prompt.ts` に表を残した）。具体か抽象かは決定的に判別できない。判定は 3 段階（`materialThickness`）で残し、eval のレポートも同じ関数で分割する — 規則を変えたときに見出しと中身がずれて前後比較が読めなくなるため。
...
## Issue #254 の実測記録（2026-09-13・書き出し・切り口・文体の候補）

本番 E2E で、書き出し「訪問のきっかけから始める」のもとで来店の経緯・動機の創作（「〇〇駅前を通る際に
立ち寄りました」）が観測された。原因の推定を実測で確定し、書き出し・切り口を素材に対して成り立つ候補からだけ選ぶ形へ改めた。

条件はすべて `eval:factuality`・`gemini-3.1-flash-lite`（本番の survey-web と同じ）・事後検証なし（プロンプト単体の効果）・
20 素材 × 5 回 × 3 本。来店の経緯・動機の創作は、この Issue で足した第二の軸（`src/lib/draft/visit-context.ts`）で数え、
表の値はすべて最終版の検出器で保存済みの下書きを数え直したものである。

### 測定した構成

| 名前 | コミット | 内容 |
|---|---|---|
| 変更前 | `9c08d1b` | 候補は変更前のまま。書き出しを 3 候補それぞれ固定して 100 回ずつ（HEAD では `EVAL_OPENING` が外した候補を受け付けないので、再現はこのコミットで行う） |
| 途中構成 | `620e449` | 候補を素材に対して成り立つものだけから選ぶ |
| 最終構成（文体 3） | `bd5dea9` | 上に加え、星の数値の読み上げと「無かった」の断定を禁じる規則を足した（独立レビューの指摘 1） |
| 文体 6 | `6222c67` | 上に加え、文体を 3 → 6（薄い素材の多様性を戻すため） |
| 採用（文体 5） | HEAD | 文体 6 から「一文ずつ短く区切った常体」を外した。値は文体 6 の実測から該当の文体を除いた部分集合（n=248）での見積もり |

### 変更前: 書き出しを固定して各 100 回

| 書き出し（固定） | 選んでいない観点への言及 | 来店の経緯・動機の創作 |
|---|---|---|
| 料理の感想から始める | 2/100 | 1/100 |
| 店の雰囲気から始める | 10/100（雰囲気を選んでいない素材の 10/85） | 1/100 |
| 訪問のきっかけから始める | 2/100 | 31/100 |

切り口でも同じ傾向があった。「味の具体性を重視」は味を選んでいない素材で 6/61、選んだ素材で 0/46 が逸脱し、
素材に依存しない「総合的な満足度を重視」は 1/112 だった。

### 構成ごとの比較

| 構成 | 観点の逸脱 | 経緯の創作 | 星の読み上げ | 「無かった」の断定 | 字数の規則内（観点あり／一言／星だけ） | 書き出しが異なる組の割合（同） |
|---|---|---|---|---|---|---|
| 変更前 | 14/300 | 33/300 | 4/300 | 0/300 | 167/180・86/90・29/30 | 97%・95%・98% |
| 途中構成 | 5/300 | 3/300 | 37/300 | 5/300 | 158/180・87/90・29/30 | 97%・90%・95% |
| 最終構成（文体 3） | 1/300 | 0/300 | 0/300 | 0/300 | 163/180・84/90・27/30 | 94%・84%・86% |
| 文体 6 | 5/300 | 1/300 | 0/300 | 0/300 | 154/180・79/90・29/30 | 97%・93%・95% |
| 採用（文体 5・見積もり） | 5/248 | 1/248 | 0/248 | 0/248 | 136/149・70/73・26/26 | 98%・91%・94% |

- 「書き出しが異なる組の割合」は、同じ素材の下書き 2 本の組のうち、店名を除いた本文の先頭 10 字が異なる組の割合（素材ごとの平均）。本数に依存しない多様性の粗い代理である
- 対照群（全観点を選んだ素材）の逸脱は、どの構成でも 0 だった
- 途中構成では、新しい書き出し「全体の満足度から始める」が星の数を読み上げる定型文と、素材の「なし」を否定の断定へ写す下書きを生んだ。規則を足した最終構成では両方とも 0 になったが、薄い素材で候補が減ったことと重なって多様性が落ちた。文の形だけを変える文体を足すと戻った
- 「一文ずつ短く区切った常体」は字数の規則の下限を 22/52 が割った（ほかの文体は 0〜5）。外した
- 採用構成の観点の逸脱 5/248 は、最終構成（文体 3）の 1/300 より多い。既存の文体（親しみやすい常体・簡潔で落ち着いた文体）から 4 件、新しい文体（体言止めを交えた敬体）から 1 件で、いずれも雰囲気への言及だった。変更前（14/300）と比べると下がっている

### 検出器の訂正（2 回）

1. 最初の版は「また近くへ立ち寄った際には、ぜひ利用したい」のような将来の仮定を経緯の創作と数えていた（途中構成で拾った 12 件のうち 9 件）。「〜た際」「〜たとき」などを除いた
2. 独立レビューで、希望の「〜たく」（「立ち寄りたくなる」）・勧誘の「〜ましょう」・一部パターンだけの除外漏れ（「通りかかった際」「用事があった際」）と、「初めて伺いました」の取りこぼしが見つかった。過去形の後置条件を `placeholders.PAST` の 1 か所で定義して全パターンで共有する形へ改め、一言の除外は下書きより広い手がかりで判定するようにした

どちらも実際の出力と指摘された形を回帰ケースとして `test/visit-context-detect.test.ts` に固定した。

### 確認していないこと

- 採用構成（文体 5）そのものの実測。上の値は文体 6 の実測の部分集合からの見積もりである
- 本番構成（事後検証あり）での値。事後検証は選んでいない観点への言及だけを見て作り直すので、来店の経緯・動機の創作は減らさない
- 多様性の厳密な測定。書き出し 10 字の比較という粗い代理だけで見た
