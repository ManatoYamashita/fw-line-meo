...
- **[Issue #62 運用知見] 生成失敗ログの最小診断情報**: `DraftError` は `API_ERROR` に限り終端例外の HTTP `status` を保持し、`/api/responses`・`/api/drafts` は共通ロガーから `errorKind`（必須）と `status`（取得時のみ）を出力する。ログフィールド型をこの2値に制限し、自由記述・プロンプト・下書き・API キーをログに混入させない。
