...
## Issue #254 の実測記録（2026-09-13・書き出しと切り口の候補）

本番 E2E で、書き出し「訪問のきっかけから始める」のもとで来店の経緯・動機の創作（「〇〇駅前を通る際に
立ち寄りました」）が観測された。原因の推定を実測で確定し、候補を素材に対して成り立つものだけから選ぶ形へ改めた。

条件はすべて `eval:factuality`・`gemini-3.1-flash-lite`（本番の survey-web と同じ）・事後検証なし（プロンプト単体の効果）。
来店の経緯・動機の創作は、この Issue で足した第二の軸（`src/lib/draft/visit-context.ts`）で数えた。

### 変更前: 書き出しを固定して各 100 回（20 素材 × 5 回）

| 書き出し（固定） | 選んでいない観点への言及 | 来店の経緯・動機の創作 |
|---|---|---|
| 料理の感想から始める | 2/100 | 1/100 |
| 店の雰囲気から始める | 10/100（雰囲気を選んでいない素材の 10/85） | 2/100 |
| 訪問のきっかけから始める | 2/100 | 30/100 |

切り口でも同じ傾向があった。「味の具体性を重視」は味を選んでいない素材で 6/61、選んだ素材で 0/46 が逸脱し、
素材に依存しない「総合的な満足度を重視」は 1/112 だった。

### 変更前後（変更後は本番と同じく、成り立つ候補からランダムに選んで 300 回）

| 指標 | 変更前（3 本の平均＝一様選択の推定） | 変更後 |
|---|---|---|
| 選んでいない観点への言及 | 14/300（4.7%） | 5/300（1.7%） |
| 来店の経緯・動機の創作 | 33/300（11.0%） | 3/300（1.0%） |
| 対照群（全観点を選んだ素材）の逸脱 | 0 | 0 |

変更前の創作 33 件のうち 30 件が「訪問のきっかけから始める」から出ていた。変更後の 3 件はすべて「初めて来店しました」型の来店歴である。

変更後の実測で、観点のある素材に対する「率直さを重視」が字数の規則（100〜200 字）を割った（14/45 が 100 字未満）。
この候補は観点の無い素材に限った。最終構成を、変更後の実測から「率直さ × 観点あり」を除いた部分集合（n=255）で見積もると、
選んでいない観点への言及 5/255（2.0%）、来店の経緯・動機の創作 2/255（0.8%）、字数の規則内は観点あり 127/135・
一言だけ 87/90・何も無し 29/30（変更前は 167/180・86/90・29/30）である。

### 検出器の誤検出の訂正

第二の軸の検出器は、最初の版で「また近くへ立ち寄った際には、ぜひ利用したい」のような将来の仮定を経緯の創作と数えていた。
変更後の実測で拾った 12 件のうち 9 件がこの形だった。「〜た際」「〜たとき」「〜た時」「〜た折」を除く形へ直し、
実際の出力を否定の回帰ケースとして `test/visit-context-detect.test.ts` に固定した。上の表の値は、直した検出器で保存済みの下書きを数え直したものである（API は呼び直していない）。

### 確認していないこと

- 最終構成そのものの実測。上の見積もりは、変更後の実測の部分集合から出した値である
- 本番構成（事後検証あり）での値。事後検証は選んでいない観点への言及だけを見て作り直すので、来店の経緯・動機の創作は減らさない
- 多様性の厳密な測定。書き出し 10 字が重複しない割合（観点あり 65%→62%・一言だけ 57%→56%・何も無し 37%→37%）という粗い代理だけで見た
