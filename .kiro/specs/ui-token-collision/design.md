...
- **生成 CSS**: 是正前 41,706 B raw / 7,519 B gzip → 是正後 41,669 B raw / **7,508 B gzip**（−37 B raw / −11 B gzip）。survey-web の `globals.css` を本番と同じ入力（プローブ注入なし）でコンパイルした実測値。
  - 設計時に記載していた「47,096 → 47,043 B」は**プローブを 19 件注入した探索用スクリプトの出力**であり本番入力の値ではなかった。実装時に本番入力で測り直して差し替えた（比較は同一方法で行うこと）。
  - **サイズ予算ガードの新設は #53 の領分**であり本仕様は測定と記録に留める
