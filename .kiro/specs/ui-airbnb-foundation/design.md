...
- **選択済みの面をアクション色で塗る部品（`checkbox` / `radio-group`）の追加、またはその
  `data-checked:bg-primary` の変更**（アクション色と危険色は色では区別できないため、
  エラー指標はリングが担う。Known Gaps 4 を参照）
- **アクション色と危険色の色相関係の変化**（どちらかが寒色側へ移ると、上の判定が過剰になる）
...
- **overlay 系（Dialog / Popover / Select / Tooltip / Tabs / Toast）は 4 面すべてで不要**
  ...
  - store-detail は操作要素ゼロが構造契約として固定されている
    - 2026-09-13 訂正（Issue #265）: store-detail は、書込の手段となる要素を描かず、入力を許可リスト（期間・指標の選択肢と競合の検索欄）に限る形へ改定した。詳細は `.kiro/specs/ui-airbnb-surfaces/requirements.md` の要件 3.1・3.3 の訂正にある。選択肢は既存の `RadioGroup` を常に見える形で使い、overlay 系は使わない。この判断の結論は変わらない
...
- **正しい赤**（期待値を追随させる）: 役割キー集合、`brand` の値、余白・角丸・影のキー集合、
  役割対応表、余白の倍率表
- **影の名前空間プローブは差し替えない**（当初は差し替える計画だった）。`shadow-*` は Tailwind が
  実値を `--tw-shadow` へ展開するため、解決先の照合は base でも current でもテーマ変数を返さず、
  プローブを替えても検出は 1 件も増えない。`shadow-raised` を使う部品が入る #174 で対照ごと立て直す
...
4. **アクション色と危険色は色では区別できない。** 出典のパレットは暖色一色であり、
   `primary #E00B41`（OKLCH 色相 18.5）と `destructive #B32505`（同 32.8）は色相差 約 14 度・
   相互コントラスト 1.35:1 しかない。差し替え前の緑と赤の組も相互比は 1.29:1 だったので、
   **両者を分けていたのは輝度ではなく色相**であり、その色相差が失われた。D2 が `success` へ入れた
   分離の不変条件はここへ適用できない（危険色を暖色から外すと「危険」に読まれなくなる）。
   影響が実際に出るのは、**選択済みの面をアクション色で塗る部品**（`checkbox` / `radio-group`）
   だけである。面が透明な部品（`input` / `textarea` / `button` / `badge`）は、エラー時に枠色が
   中立の灰から赤へ変わるため枠が信号を運べる。前者は面が既にアクション色なので枠が信号を
   運べず、残る手掛かりは `aria-invalid` のリングだけになる。shadcn 既定の `/20` は対白 1.41:1 で
   SC 1.4.11 の 3:1 に届かないため、この 2 部品に限り `/60`（対白 3.09:1）へ引き上げた。
   実効性は `@fwlm/ui` の `test/contrast-usage.test.ts` が**部品ソースから段を引いて**検証する
   （表へ書き写すと部品を戻しても緑のままになる。実測で確認済み）。
   受容の前提は `@fwlm/design-tokens` の `test/colors.test.ts` が固定する
5. **`--text` 名前空間に段を足していない。後続 spec は既定スケールの大きい段をそのまま使える**
