...
- 7.2（最終実証・2026-07-27）: **性能は基準線から増分ゼロ** — client JS 182.8 KB gzip / 300 KB（タスク1.3 の基準線と同値。Tailwind v4 はランタイム JS ゼロ・部品は未 import のため）。**生成 CSS は 33,278 B raw / 6,598 B gzip**（perf:budget は JS のみ計測のため別途実測・6.1 レビュアー申し送りに対応）。部品を実 import した場合の実測は 196.1 KB（+13.3 KB・タスク6.3 計測）で予算に余地 103.9 KB
  - **【2026-08-02 追記・ui-token-collision タスク 5.2 による是正】** 上記の生成 CSS サイズは 2026-07-27 時点の値であり、その後 `/ui-check`（検証専用面）の追加と ui-a11y-gaps の実装により増加している。**同一方法（survey-web の globals.css を本番と同じ入力でコンパイル）による 2026-08-02 の実測は 41,669 B raw / 7,508 B gzip**（ui-token-collision の是正後）。client JS は同日 **213.6 KB gzip / 予算 300 KB**。**サイズは計測方法（プローブ注入の有無・対象アプリ）で変わるため、比較する際は必ず同一方法で測り直すこと。** サイズ予算ガードの新設は #53 が扱う。
...
- 【フィーチャ検証 NO_GO の是正・2026-07-27】独立検証で3件の指摘 → 全て是正・再検証済み:
  1. **[High・実害のある回帰]** Tailwind Preflight が `h1`〜`h6` を `font-size: inherit` にリセットするのに補償が無く、**store-detail/dashboard-web の見出しが本文と同サイズで描画されていた**（基盤導入前より劣化）。→ `heading.tsx` 新設（要件2.1 の「見出し」充足）＋ **theme.css の `@layer base` で h1〜h6 の既定をトークン参照で復元**（アプリ DOM は不変）。生成 CSS に `h1{font-size:var(--text-2xl);font-weight:700}` が出ることを実ビルドで確認
  2. **[Medium・ガードの空振り]** theme-sync が**集合包含**しか見ておらず、役割→値の誤割当（`--color-text` を別役割の値へ）や design-tokens 単独変更（同期忘れ）を検出できなかった（同じ hex が別役割にも存在するため集合に残る）→ **役割対応表による厳密一致＋網羅ガード**に変更。4種のミューテーション（誤割当・同期忘れ・キー追加・変数追加）で赤化を実証
  3. **[Medium・要件1.4 部分未充足]** ガードが hex しか見ておらず **`bg-red-500` 等の生 Tailwind パレット色クラスを素通り**させていた → 検出を追加（走査対象を `ts/packages/ui/src/**` へ拡大）。注入で exit 1・復元で緑を実証
  - 結果: ui テスト 83→**106**・生成 CSS +189 B gzip・client JS は 182.8 KB で**増分ゼロ**を維持
