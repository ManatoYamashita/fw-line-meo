...
- [x] 1.3 (P) line-webhook アプリ雛形（Hono・Node22・config検証）
  - `package.json`/`tsconfig.json`/`Dockerfile`（node:24-slim）、`config.ts`（`LINE_CHANNEL_ID`/`LINE_CHANNEL_SECRET`/`PLACES_API_KEY`/`LINE_RICHMENU_COMPLETED_ID`/`PORT` の必須検証）、`index.ts` skeleton、`GET /healthz`
