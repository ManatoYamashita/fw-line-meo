...
- **2026-08-29: task 7.1 の「能力の不在」チェックを CI 常設化（Issue #158 (a)・PR #161・commit `0e9947ec`）**: task 7.1 が新設した `db/test/check_no_optional_capabilities.sh`（Req 1.4 = 競合リスト再抽出・調整手段を提供しない／Req 3.10 = 配信オプトアウト手段を提供しない）は、**#158 までどのワークフローからも実行されていなかった**。唯一の呼出元は `db/test/cross_runtime_steps.sh:46` で、入口は `make cross-runtime-test` という手動操作だった（`make db-test` にも `ts-ci` にも載っていない）。`scripts/run-db-test-suites.sh`（#156）の SKIP 宣言を回収して RUN 表へ移し、ts-ci の `lint-build-test` で毎 PR 実行されるようにした。
  ...
  - **残り**: Issue #158 の (b)（`cross_runtime_integration.sh` の CI 化）は未着手のため #158 は open のまま。`check-grep-exit-codes.sh` / `check-shell-pipe-consumers.sh` の走査対象が `scripts/` のみで `db/test/*.sh` が機械強制の外にある件（`check_docs.sh` に `|| true` 3 件と `printf | grep -q` 2 件が残存）は **Issue #162** として起票した。
