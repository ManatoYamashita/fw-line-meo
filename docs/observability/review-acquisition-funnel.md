...
| 投稿導線の押下 | 事象 `survey_review_link_opened` | 同上 | token を検証できた押下だけを数える。回答済み画面を開いて 5 分を超えてからの押下と、通知の送達に失敗した押下は数えない（下振れ）。同じ token の連打はレート制限まで区別しない（上振れ） |
| 口コミの増分 | 自店の `rating_snapshots.review_count` の日次差 | DB | 自店のすべての口コミを含む（本アンケートを経由しないものも含む）。削除で減る日がある |
...
`(CHANGE_AT + 18 時間)` の JST 日付の区間で、この 1 区間だけを前後どちらの窓にも入れない。
...
SELECT CASE WHEN d.captured_on < k.straddle THEN '前' ELSE '後' END AS 窓,
       ...
       sum(d.delta)   AS 増分,
       min(d.captured_on) AS 最初, max(d.captured_on) AS 最後
FROM d, k
...
`区間数` が `DAYS` に満たない窓は、日次バッチが走らなかった日があるか、窓がまだ終わっていない。
増分は窓の中の差分の合計なので、欠けた日の増分は隣の日の差分へ寄る（合計は変わらない）。
...
- 「押下 / 送信」も下振れしうる（回答済み画面の 5 分・送達失敗）。一方で同じ token の連打は上振れさせる
