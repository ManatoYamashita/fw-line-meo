...
| `text` | 1 | `background` | 1 | `background` | `#FFFFFF` | `#222222` | 15.910 | AA | 店舗詳細の順位の巨大表示と、客向けアンケートの選択済みの星 |
| `textMuted` | 1 | `background` | 1 | `background` | `#FFFFFF` | `#6A6A6A` | 5.409 | AA | 客向けアンケートの未選択の星 |
...
**管理ダッシュボードの帯の高さは `h-20` に固定する。** 原典の `top-nav` の高さ（80px）と一致する。
名前付きキーを宣言せず数値スケール（基数 0.25rem・§3）で指定するため、余白の段を増やさずに解決する。
