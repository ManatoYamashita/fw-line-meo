...
ブランド色を載せるのはアプリ名だけで、面の役割（「管理用ダッシュボード」）は補足の色で添える。アプリ名は
狭い画面で 20px・広い画面で 24px の太字とし、どちらも大きい文字の閾値（14pt の太字）を越える。狭い画面では
面の役割が次の行へ回り、アプリ名は語の切れ目でだけ折れる。表示名は 3 アプリとも「Firstweb 集客AIアシスタント」＋
