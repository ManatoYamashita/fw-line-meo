...
既存の `ui-foundation.spec.ts` の再生成中表示の計測で、モック応答が先に完了すると
spinnerが消えて失敗する競合を2回再現した。リクエストをPromiseで保留し、描画を計測した後に
finallyで解放することで、固定sleepを増やさず検証対象の状態を保つ。
LPテストの追加で処理中表示の検証を削除したり、アサーションを緩めたりしない。

修正後、一時DB・Geminiモックの本番ビルドでsurvey全体E2E **57件成功**（31.6秒）。
LP 6件・店舗回答から下書きコピーまでの導線・noindex継承・部品の実描画を含む。

CIでLPの幅検証が `page.goto` のload待ちで30秒を超えたため、初期HTMLのDOMContentLoadedと
本文の可視を開始条件に変更した。幅検証は同じ画面をリサイズして計測し、幅を変えるたびの
不要な再ナビゲーションを削除する。画像のロード成功は画像専用の実描画確認で別途確認する。
