...
実装コード・`package.json`・`go.mod` はまだ存在しない（`.gitignore` は Node/TS + Go + GCP/Terraform を想定済み）。
ビルド・lint・テストのコマンドは最初のコード導入時に確立し、本ファイルへ追記すること。**未確立のコマンドを推測で書かない。**
...
- Steering: `.kiro/steering/`（プロジェクト全体の永続メモリ。`/kiro-steering` で生成）
