...
- `make e2e`（`bash scripts/run-e2e-local.sh`）— 自動層をローカルで一括実行する。層は `survey`（客向け画面の Playwright・一時 DB＋seed＋Gemini モック）／`surfaces`（管理ダッシュボードと店舗詳細の Playwright・IdP スタブのビルド）／`lighthouse`（LCP 要素が seed の店名であることまで確かめる）／`cross-runtime`。`--only survey,lighthouse` で絞れる。ts-ci の同名ジョブと同じものを流す。node 24 への切り替え・ポートの先客検出・スタブ入りビルドの巻き戻しは装置の側で行う。集約実行
- `PROJECT_ID=gen-fw-line-meo make e2e-prod-checks`（`bash scripts/run-e2e-prod-checks.sh`）— 本番の読み取り専用の確認（稼働イメージ・シークレット・実疎通の鮮度・**本番のコミットに対する ts-ci の E2E ジョブ**・日次ジョブ・配信の件数）。**運用者用・CI からは呼ばない**（ログを読むため）。本番の状態は変えない
