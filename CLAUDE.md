...
- `scripts/check-jst-offset-consistency.sh` — JST の固定 `+09:00` と整形方式の整合検証（Issue #299・ts-ci）。TS 4 実装（暦日 3・投稿時刻 1）と Go 1 実装の値が 32,400 秒に一致すること、SQL の `AT TIME ZONE 'Asia/Tokyo'`、宣言と実装候補の双方向一致を検証する。**テストコードは本番の時刻決定権を持たないため走査対象外**。外部サービス・実行時刻・実行環境の TZ へ依存しない
