...
- 検証用のテナントを使い回し、実機確認のために新しい店舗・オーナーを作らない。本番の店舗・
  オーナーを削除する経路は無く、作った店舗を毎朝の取得と配信の対象から外すには停止（Issue #252。
  dashboard-web の `/stores` の「利用状況」列の「停止」ボタン。運営は全店舗、代理店は担当店舗のみ）
  が要る。停止しても店舗とオーナーの行は残る。
...
オーナーが実在することが確かめられている。本番の店舗・オーナーを削除する経路は無く、店舗の停止
（Issue #252）も行を残したままオーナーの完了状態を変えないので、対象が消えて無くなることもない。したがって**段 3 の張り替えは省略できない。** 対象が 1 人でも、
