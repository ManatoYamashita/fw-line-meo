...
// - 検索欄は色を書かない。検索欄の色は部品の側がトークンから解決し、件数の文言は本文色を継承する（§7.19。
//   店舗詳細の面で色を書くのは推移グラフの部品だけである）。
//
...
//
// このファイルは部品単体の検査だけを持つ。ページ全体での検索の検査は、ページ全体のテストファイル
// （test/trend-dashboard.test.tsx・タスク 4.2）が持つ。
...
describe('CompetitorSearch の検索欄（Issue #265）', () => {
  ...
  it('縦に積む Field の中に、ラベル・入力欄の順で置く（§7.19）', () => {
    const { container } = renderPlain({ total: 5, visibleCount: 5 });
...
describe('CompetitorSearch の件数の文言（Issue #265）', () => {
  ...
  it('入力された検索語を、画面のどこにも表示し直さない（§7.19・要件 6.4）', () => {
    renderWithFilter();
...
describe('CompetitorSearch の色（Issue #265）', () => {
  it('面が足すクラスは、並べ方と件数の文言の文字の段だけで、部品に色を足さない（§7.19）', () => {
    const { container } = renderPlain({ total: 5, visibleCount: 2 });
