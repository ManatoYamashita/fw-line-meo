...
// テスト専用ヘルパー（ui-airbnb-surfaces task 3.1）。
//
// dashboard-web の test/live-region.ts と同一の実装である。**面をまたぐコードを作らない**という
// design.md「Architecture Pattern and Boundary Map」の方針に従い、共有パッケージへは切り出さずに
// 面ごとに置く（テスト専用モジュールであり、vitest の既定 include に載らないので単体では
// 実行されない）。共有すべきと判断するなら、それは本 spec ではなく部品層側の課題である。
//
