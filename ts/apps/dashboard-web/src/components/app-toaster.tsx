...
// 状態は左上のにじみとアイコンの意味色（globals.css）と文言で示す。文字を意味色で塗らないのは、
...
// 成功・失敗の Toast の左上の角から左辺に沿って置くにじみ（globals.css の [data-slot='toast-accent']）。
// **疑似要素ではなく本物の要素で描く。** axe は、文字の祖先が疑似要素に塗りを持つと対比の判定を
// 「判定不能（pseudoContent）」へ降ろし、違反 0 件のまま Toast の文字の監査が消える。本物の要素は、
// 文字の位置に重なっていなければ背景の判定に加わらない。Toast は左の内側余白を広げて文字を
// にじみの右へ送るので、文字とにじみは重ならない。にじみは装飾なので読み上げない。
