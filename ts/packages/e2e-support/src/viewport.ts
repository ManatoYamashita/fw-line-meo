// 実描画で「面が横スクロールしないこと」を測るための共有ヘルパ（Issue #53）。
//
// もとは ts/apps/survey-web/e2e/ui-foundation.spec.ts のモジュール private だった。管理
// ダッシュボードと店舗詳細へ実ブラウザ検証の足場を入れるにあたり、**同じ除外規則を 3 面が
// 共有する**必要が生じたため、挙動を変えずにここへ移した。複写にしなかったのは、除外規則の
// 修正が 3 箇所へ分散し、片方だけ古びても誰も検出できないためである（それは Issue #53 が
// 問題にしている構造そのものである）。
//
// 移設にあたって規則・しきい値・メッセージの文言は 1 文字も変えていない。survey-web の
// 既存の呼び出し 4 箇所と対照 6 件が、件数も文言も変えずに緑であることが移設の完了条件である。
...
export const PANNABLE_OVERFLOW_X: readonly string[] = ['auto', 'scroll'];
