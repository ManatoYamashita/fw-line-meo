 ...
 *   - `jsonPayload.event = "survey_page_viewed"` / `"survey_response_submitted"` /
 *     `"survey_review_link_opened"`（Issue #137）で絞り込み
...
describe('本番の集計指標が読む契約', () => {
  // survey_funnel の filter が絞り込む 3 事象（infra/modules/guardrails/main.tf の for_each と同じ集合）。
  for (const event of ['survey_page_viewed', 'survey_response_submitted', 'survey_review_link_opened']) {
