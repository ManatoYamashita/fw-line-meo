# 書き込み境界（Write Boundary）: four-tier-data-model
...
| `owners` | TS リアルタイム応答層 | LINE オンボーディング（Webhook） |
...
- 読み取りは両層に許容するが、書き込みは責任層のみ。クロス言語の典型 seam は「Go が `rating_snapshots`/`competitors` を書き、TS が日次サマリー配信で read」。
