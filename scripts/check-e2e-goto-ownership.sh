...
#   1. `ts/apps/*/e2e/` 配下の **Playwright が収集しうる拡張子**のファイルのうち、`fixtures/` の
#      内側に無いものが `page.goto(` を 1 件も持たない（spec だけでなく helper・stub も対象。
#      所有者を 1 種類に絞るため）
#   2. 例外は WHITELIST へ理由と Issue 番号つきで明記する（下記）。載っているのに違反が
#      無くなったものは警告して削除を促す
#   3. 空振り防止: 走査対象が 0 件なら赤。加えて **走査対象を持つアプリごとに**、その
#      `e2e/fixtures/` が `page.goto(` を 1 件以上持つことを要求する
#      （**後者が重要である。** 「誰も面を開かなくなった」状態は、禁止が守られている状態と
#        区別が付かないまま緑を返す。**合算で数えてはならない** —— あるアプリの所有者が
#        goto を失っても別アプリの分で埋まり、そのアプリの空振りが見えなくなる）
#   4. 走査が評価不能（grep exit 2 以上）だったファイルは fail-closed で赤にする。
#      「読めなかった」を「違反 0 件」とも「所有者あり」とも読まない
#
...
# 走査対象の拡張子。Playwright の既定 testMatch は `**/*.@(spec|test).?(c|m)[jt]s?(x)` なので、
# `.ts` だけに絞ると `.spec.mts` や `.spec.tsx` へ 1 段隠した goto が **Playwright には収集され
# つつ本ガードからは見えない**。禁止の抜け道が拡張子ひとつで開くため、収集されうる形をすべて見る。
SCAN_EXTS='ts tsx mts cts js jsx mjs cjs'
...
# find へ渡す name 式。`-o` 区切りの配列として組み立て、括弧でまとめて `-type f` と併用する。
FIND_NAME_ARGS=()
for scan_ext in $SCAN_EXTS; do
  [ "${#FIND_NAME_ARGS[@]}" -eq 0 ] || FIND_NAME_ARGS[${#FIND_NAME_ARGS[@]}]='-o'
  FIND_NAME_ARGS[${#FIND_NAME_ARGS[@]}]='-name'
  FIND_NAME_ARGS[${#FIND_NAME_ARGS[@]}]="*.${scan_ext}"
done
if [ "${#FIND_NAME_ARGS[@]}" -eq 0 ]; then
  echo "ERROR: SCAN_EXTS が空です。走査対象の拡張子が 1 つもありません。" >&2
  exit 1
fi
...
# 評価不能は負値で返し、判定は呼び出し側が行う。
#
# **この関数の中で fail を立ててはならない。** 呼び出しはコマンド置換（副シェル）なので、
# 代入した値は親へ戻らず、stderr のエラーだけが出て exit 0 という**偽の緑**になる。
# 実際この形で 1 度踏んだ（PR #192 のレビュー指摘 1・実測で `OK` / exit 0 を再現し、
# fixtures 側で起きたときは件数へ -1 が混ざって空振り防止まですり抜けた）。
# steering `tech.md`「シェルガードの実装規律」が同じ形を禁じている。
count_fixed() {
  cf_rc=0
...
for app_path in "$APPS_DIR"/*/; do
  ...
  e2e_files="$(find "$e2e_dir" -type f \( "${FIND_NAME_ARGS[@]}" \))"
  [ -n "$e2e_files" ] || continue

  app_rel="${app_path%/}"
  app_rel="${app_rel#$ROOT/}"
  # **アプリ単位で数える。** 合算だと、あるアプリの所有者が goto を失っても別アプリの分で
  # 埋まり、そのアプリの空振りが見えなくなる。
  app_scanned=0
  app_fixture_goto=0
  ...
  for f in $e2e_files; do
    ...
    # **判定はここで行う**（count_fixed の注記を参照）。負値は「走査できなかった」であり、
    # 「違反 0 件」でも「所有者あり」でもない。どちらの母数へも混ぜず fail-closed にする。
    if [ "$n" -lt 0 ]; then
      echo "ERROR: ${rel} を走査できなかったため判定できません。" >&2
      echo "       → 判定不能を「違反なし」とも「所有者あり」とも読みません。" >&2
      fail=1
      continue
    fi
    case "$f" in
      */fixtures/*)
        ...
        app_fixture_goto=$((app_fixture_goto + n))
  ...
  # アプリ単位の空振り防止（合算では別アプリの分で埋まる）。
  if [ "$app_scanned" -gt 0 ] && [ "$app_fixture_goto" -eq 0 ]; then
    echo "ERROR: ${app_rel} に走査対象 ${app_scanned} 件がありますが、その e2e/fixtures/ に ${GOTO_MARK} が 1 件もありません。" >&2
    echo "       → 「このアプリでは誰も面を開かなくなった」状態は、禁止が守られている状態と" >&2
    echo "         区別が付きません。所有者が実在することまで要求して初めて検査になります。" >&2
    fail=1
  fi
done
...
if [ "$scanned_count" -eq 0 ]; then
  echo "ERROR: ts/apps/*/e2e/ に走査対象のファイルが 1 件もありません。ガードが空振りしています。" >&2
  exit 1
fi
