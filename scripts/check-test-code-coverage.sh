...
# 非ゼロが `set -e` に拾われてスクリプトごと止まる。
#
# **この保護は「呼び出し元が条件文脈でないこと」に依存する。** `check_root_files` や
# `check_subdir_files` を `if ...` や `... || fail=1` へ移すと `set -e` が関数全体で無効に
# なり、13 箇所すべてが空文字を 0 と読む形 — 本 PR が撤去した後置 `true` と同じ挙動 — へ
# 黙って戻る（PR #123 レビューで実測）。不変条件は 10 番の変異ケース
# 「関数内の評価不能が呼出元まで伝播して赤になる」が固定しており、条件文脈へ移すと落ちる。
count_matches() {
  cm_rc=0
  cm_out="$(grep -c "$@")" || cm_rc=$?
  if [ "$cm_rc" -gt 1 ]; then
