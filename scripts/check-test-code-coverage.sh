...
# 担当域のコードファイルとして数えた件数（深さで落とす前）。checked_subdir_files が 0 に
# なったときに「走査系が壊れた」のか「対象が消えた」のかを機械的に切り分けるために持つ（Issue #81）。
found_subdir_candidates=0
...
# `.ts` は CODE_DIR_CANDIDATES のディレクトリ走査が担当済みで、広げると二重報告になる。
subdir_ext_in_scope() {
  case "$1" in
...
# ---------------------------------------------------------------------------
#
# 呼出元の fail / checked_subdir_files / found_subdir_candidates を更新する
# （サブシェルを挟まないこと）。
#   $1 対象ディレクトリ（末尾 / 付きの絶対パス）
#   $2 表示用の相対パス（末尾 / 付き）
#   $3 その単位の lint スクリプト文字列
...
check_subdir_files() {
  ...
  while IFS= read -r csf_path; do
    ...
    # (i) 別の呼出が既に担当した領域か（絶対パス接頭辞で判定する。理由は関数の定義箇所を参照）。
    subdir_owned_elsewhere "$csf_path" "$csf_skip_dirs" && continue

    # (ii) この呼出が見るべき拡張子か。
    subdir_ext_in_scope "$csf_path" "$csf_exts" || continue

    # ここまで残ったものが「この呼出が担当すべき候補」。**深さで落とす前に数える。**
    # find の生の出力で数えると、ts/ 直下呼び出しが workspace 配下のぶんまで抱え込んで
    # 常に 200 超になり、下の「find が何も返していない」分岐が到達不能になる。
    found_subdir_candidates=$((found_subdir_candidates + 1))

    # (iii) 直下のファイルは check_root_files の担当（二重報告しない）。
...
# find が返した候補の件数を別に持ち、走査系の破損と対象の消失を機械的に切り分ける。
if [ "$checked_subdir_files" -eq 0 ]; then
  if [ "$found_subdir_candidates" -eq 0 ]; then
    echo "ERROR: サブディレクトリ走査でコードファイルを1件も発見できませんでした。ガードが空振りしています。" >&2
    echo "       → 走査系の破損が濃厚です。prune 一覧が広すぎるか find の式が壊れています" >&2
    echo "         （Issue #83 の走査）。直下ファイルは ${checked_root_files} 件検証できているため、" >&2
    echo "         リポジトリからコードが消えたわけではありません。" >&2
  else
    echo "ERROR: サブディレクトリのコードファイルが0件です（発見 ${found_subdir_candidates} 件はすべて直下または workspace 側の担当）。" >&2
    echo "       → 走査系は生きています。対象ファイルが実際に無くなった可能性が高いです" >&2
    echo "         （直近の占有者は survey-web の perf/*.mjs と e2e/*.mjs）。" >&2
    echo "         占有者ゼロは正常状態になり得ますが、本ガードは fail-closed のため赤にします。" >&2
    echo "         意図した消失であれば、本チェック自体の要否を見直してください。" >&2
  fi
  exit 1
fi
