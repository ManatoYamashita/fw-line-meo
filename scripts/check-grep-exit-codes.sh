...
#      （除外した違反行の内容そのものがソースへ現れるため。全ファイルへ広げると、複数行
#      `awk` / `sed` の閉じ引用符から始まる行の違反が痕跡なく消える。PR #119 と同型）
#   4. 空振り防止: 走査ファイル 0 件・`grep` を含む行 0 件はいずれも赤
#   5. WHITELIST の項目が 1 件も当たらなくなったら WARNING を出す
#
