...
#      全ファイルへ広げると、複数行 `awk` / `sed` の閉じ引用符から始まる行へ置かれた違反が
#      ERROR も SKIP も出さずに消える（PR #119 レビュー指摘）
#   4. 空振り防止: 走査ファイル 0 件・パイプを含む行 0 件はいずれも赤
#      （列挙や走査パターンの前提が崩れたまま「違反 0 件だから緑」を返さない）
#   5. WHITELIST の項目が 1 件も当たらなくなったら WARNING を出す（無意味な除外を残さない）
#
...
# `$0` から導く。改名しても WHITELIST 宣言は本体と一緒に動くため、追随が要らない。
...
scan() {
  ...
  while IFS= read -r line; do
    ...
    # 行頭が `#` のコメント行は全ファイルで対象外。規律や過去の失敗を説明する注記が、まさに
    # この構文を引用するため（例: 「この形は SIGPIPE で偽陽性を生む」と書いた注記そのもの）。
    case "$trimmed" in
      '#'*) continue ;;
    esac

    # WHITELIST の宣言行と、配列要素が行ごとに並ぶ書き方の引用符行は、コマンドではなくデータ
    # である。違反行を WHITELIST へ載せると **その行の内容がガード本体のソースへ現れる**ため、
    # 除外しないと本ガードは「除外した違反」を自分の中に見つけて永久に赤くなる（自己テストで実測）。
    #
    # **この除外は WHITELIST を宣言する本ファイルへ限定すること。** 全ファイルへ広げると、
    # 複数行の `awk` / `sed` で残りのコマンドが続く閉じ引用符行 — 本リポジトリに 3 箇所実在する
    # 形である — に置かれた違反が、ERROR も SKIP も出さずに消える（PR #119 レビュー指摘）。
    # 除外の広さは「入れた理由」ではなく「外したら赤くなるか」で測ること。対照は 85 番にある。
